...
**`utility_kit`** — framework-agnostic helpers:

```
packages/utility_kit/lib/src/
├─ base/          # base classes (e.g. base cubit/state, failures)
├─ exception/
├─ extension/
├─ localization/
├─ service/
...
1. `packages/<name>/` with `pubspec.yaml` (`resolution: workspace`), barrel `lib/<name>.dart`, one-line `analysis_options.yaml`.
