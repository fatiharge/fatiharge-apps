...
- **Comments in code** — sparse, and the same anti-drift rule applies. A comment earns its place by saying *why*, and only where the reason is not visible from the code: a decision that looks wrong until explained, a constraint the platform imposes, a trap the next reader would fall into. Anything that restates what a line does, or repeats a value the code already holds, starts lying the first time the code moves. Prefer a clearer name or a test over a paragraph; if it must be said, say it in one or two lines.
